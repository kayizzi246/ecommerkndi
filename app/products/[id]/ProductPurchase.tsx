...
export default function ProductPurchase({
  ...
  ratingCount = 0,
  ratingBreakdown = [],
  /** The shop WhatsApp number from wp-admin; blank hides the WhatsApp order button. */
  whatsapp = "",
  /** This product's full URL, put into the WhatsApp message. */
  productUrl = "",
  /**
    ...
    <>
      ...
      <div className="mx-auto flex max-w-[1360px] flex-col gap-4 rounded-lg bg-white p-0 lg:grid lg:grid-cols-[46%_1fr] lg:items-start lg:gap-x-10 lg:gap-y-6">
        ...
        <div className="w-full lg:sticky lg:top-[112px] lg:col-start-2 lg:row-span-2 lg:row-start-1 lg:self-start">
          ...
          {/* ---- Order on WhatsApp ----
               Many shoppers here would rather ask and order by chat than fill
               in a checkout. This opens WhatsApp with the product, its price and
               its link already typed, so the shop knows exactly what is meant
               and the shopper only has to press send. Hidden until a WhatsApp
               number is set in wp-admin, rather than pointing at nobody. */}
          {whatsapp.replace(/[^\d]/g, "") && (
            <a
              href={`https://wa.me/${whatsapp.replace(/[^\d]/g, "")}?text=${encodeURIComponent(
                `Hello Kandi, I'd like to order:\n${product.name}\n${formatPrice(product.price)}\n${productUrl}`
              )}`}
              target="_blank"
              rel="noopener noreferrer"
              className="mt-3 flex items-center justify-center gap-2.5 rounded-xl border-2 border-[#25d366] bg-[#f0fdf4] px-4 py-2.5 text-[#128c4a] transition-colors hover:bg-[#dcfce7]"
            >
              <svg aria-hidden className="h-5 w-5 shrink-0" fill="currentColor" viewBox="0 0 24 24">
                <path d="M17.47 14.38c-.3-.15-1.76-.87-2.03-.97-.27-.1-.47-.15-.67.15-.2.3-.77.97-.94 1.17-.17.2-.35.22-.64.07-.3-.15-1.26-.46-2.4-1.48-.89-.79-1.49-1.77-1.66-2.07-.17-.3-.02-.46.13-.61.13-.13.3-.35.45-.52.15-.17.2-.3.3-.5.1-.2.05-.37-.02-.52-.08-.15-.67-1.61-.92-2.2-.24-.58-.49-.5-.67-.51h-.57c-.2 0-.52.07-.79.37-.27.3-1.04 1.02-1.04 2.48s1.07 2.88 1.21 3.08c.15.2 2.1 3.2 5.08 4.49.71.31 1.26.49 1.69.63.71.23 1.36.2 1.87.12.57-.08 1.76-.72 2.01-1.42.25-.7.25-1.29.17-1.42-.07-.12-.27-.2-.57-.35M12.05 21.5h-.01a9.4 9.4 0 0 1-4.8-1.31l-.34-.2-3.57.93.95-3.48-.22-.36a9.4 9.4 0 0 1-1.44-5.02c0-5.2 4.23-9.43 9.44-9.43a9.37 9.37 0 0 1 6.67 2.77 9.37 9.37 0 0 1 2.76 6.67c0 5.2-4.24 9.43-9.44 9.43m8.03-17.46A11.28 11.28 0 0 0 12.05.72C5.8.72.7 5.8.7 12.07c0 2 .52 3.95 1.52 5.67L.6 23.28l5.67-1.49a11.3 11.3 0 0 0 5.78 1.47h.01c6.26 0 11.35-5.09 11.35-11.35 0-3.03-1.18-5.88-3.33-8.02" />
              </svg>
              <span className="flex flex-col leading-tight">
                <span className="text-[14px] font-bold">Order on WhatsApp</span>
                <span className="text-[11px] font-medium text-[#15803d]">Ask a question or order by chat, we reply fast</span>
              </span>
            </a>
          )}
