...
export default function ContactForm({ supportPhone }: { supportPhone: string }) {
  ...
  if (sent) {
    return (
      ...
      >
        <p className="text-[18px] font-semibold text-shop-success">Message sent</p>
        <p className="mx-auto mt-2 max-w-md text-[14px] text-shop-body">
        ...
        <button
          ...
          className="btn-shop-outline mt-5 px-6 py-2.5 text-[14px]"
        >
          Send another
        </button>
      </div>
  ...
  return (
    <form onSubmit={submit} className="rounded-2xl border border-shop-line bg-white p-6">
      <h2 className="text-[19px] font-extrabold text-shop-ink">Send us a message</h2>
      ...
      <label className="mt-4 block">
        <span className="mb-1.5 block text-[13px] font-semibold text-shop-body">Message</span>
        <textarea
          required
          rows={5}
          minLength={10}
          ...
          className="field-shop resize-y text-[14px]"
        />
      </label>
      ...
      <button type="submit" disabled={busy} className="btn-shop mt-5 px-8 py-3 text-[14px]">
