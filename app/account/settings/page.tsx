...
export default function AccountSettings() {
  ...
  return (
    <div>
      <h1 className="text-[20px] font-extrabold leading-tight text-shop-ink">Settings</h1>
      ...
      <section className="mt-6 rounded-2xl border border-shop-line bg-white p-4 sm:p-5">
        <h2 className="text-[18px] font-extrabold text-shop-ink">Profile</h2>
        <div className="mt-4 flex items-center gap-4">
          {customer.avatar ? (
            ...
            <span className="flex h-16 w-16 items-center justify-center rounded-full bg-shop-primary-soft text-[18px] font-semibold text-shop-primary">
              {customer.name.charAt(0).toUpperCase()}
            </span>
          )}
          <div className="min-w-0">
            <p className="truncate text-[16px] font-semibold text-shop-ink">{customer.name}</p>
            <p className="truncate text-[14px] text-shop-muted">{customer.email}</p>
      ...
      <form onSubmit={save} className="mt-5 rounded-2xl border border-shop-line bg-white p-4 sm:p-5">
        <h2 className="text-[18px] font-extrabold text-shop-ink">Delivery preferences</h2>
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="block">
            <span className="mb-1.5 block text-[13px] font-semibold text-shop-body">
              Usual size
            </span>
            <input
              value={size}
        ...
        <div className="mt-5 flex flex-wrap items-center gap-4">
          <button type="submit" disabled={busy} className="btn-shop px-8 py-3 text-[14px]">
            {busy ? "Saving…" : "Save changes"}
          </button>
          {status === "saved" && (
            <span role="status" className="text-[13px] font-medium text-shop-success">
              Saved to your account.
            </span>
          )}
          {status === "error" && (
            <span role="alert" className="text-[13px] font-medium text-shop-sale">
