...
export default function SellerOrdersPage() {
  ...
  const acceptCell = (order: SellerOrder) =>
    CLOSED_STATUSES.includes(order.status) ? (
      <span className="text-[12px] text-bfl-grey">—</span>
    ) : order.accepted ? (
      <span className="whitespace-nowrap text-[12px] font-semibold text-[#0a7a2f]">✓ Accepted</span>
    ) : (
      <button
        type="button"
        onClick={() => accept(order)}
        disabled={accepting === order.id}
        className="whitespace-nowrap rounded border border-black bg-black px-3 py-1.5 text-[12px] font-semibold text-white transition-opacity hover:opacity-85 disabled:opacity-50"
      >
    ...
    <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_320px]">
      <div>
        <div className="mb-2 flex flex-wrap items-baseline justify-between gap-2">
          <p className="text-[12px] font-semibold text-black">Your items in this order</p>
          <button
            type="button"
            onClick={() => printPackingSlip(order, seller?.store_name ?? "Kandi")}
            className="rounded border border-bfl-line bg-white px-3 py-1.5 text-[12px] font-semibold text-[#333] transition-colors hover:border-[#b0b0b0]"
          >
            Print packing slip
          </button>
        </div>
  ...
  return (
    <div className="mx-auto max-w-[1200px]">
      <h1 className="text-[25px] font-extrabold text-black">Orders &amp; sales</h1>
      ...
      <div className="mb-4 mt-5 flex flex-wrap items-center gap-3">
        ...
        <input
          ...
          className="min-w-[220px] flex-1 rounded border border-bfl-line px-3 py-1.5 text-[13px] text-[#333] placeholder:text-bfl-grey focus:border-[#b0b0b0] focus:outline-none"
        />
      </div>
      ...
      {notice && (
        <p className="mb-4 border-l-2 border-[#0a7a2f] bg-[#e7f7ea] px-3 py-2 text-[13px] text-[#0a7a2f]">
          {notice}
        </p>
      )}
        ...
        <div className="mb-4 grid gap-4 sm:grid-cols-3">
          {[
            ...
            <div key={label as string} className="rounded border border-bfl-line bg-white px-4 py-3">
              ...
              <p className="mt-1 text-[19px] font-semibold text-black">
      ...
      <div className="space-y-3 md:hidden">
        {orders === null && (
          <p className="py-12 text-center text-[13px] text-bfl-grey">Loading…</p>
        )}
          ...
          <div key={order.id} className="rounded border border-bfl-line bg-white">
            ...
            >
              <div className="min-w-0">
                <p className="text-[14px] font-semibold text-bfl-ink">#{order.number}</p>
                <p className="mt-0.5 truncate text-[13px] text-black">{order.customer}</p>
                <p className="text-[12px] text-bfl-grey">
                  {order.city}
              ...
              <div className="shrink-0 text-right" style={{ fontVariantNumeric: "tabular-nums" }}>
                ...
                <p className="mt-0.5 text-[12px] text-bfl-grey">
